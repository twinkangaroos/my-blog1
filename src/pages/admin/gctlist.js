...
const GctList = () => {
    ...
    useEffect(() => {
        ...
        DataStore.observe(GctNews).subscribe(()  => {
            doInit()
        })
    }, [])
    ...
    async function doInit() {
        const items = await DataStore.query(GctNews, Predicates.ALL, {
    ...
    const nextButtonClick = (p_id) => {
        // <Link href={`/admin/gct/update/${post.id}`}>編集</Link>
        router.push(`/admin/gct/update/${p_id}`)
    }

    // 記事作成ボタンクリックで画面遷移
    const createButtonClick = (p_id) => {
        router.push(`/admin/gct/create_post`)
    }
                        ...
                        <TableBody>
                        {
                            gctnews.map(news => (
                                <TableRow key={news.id}>
                                    <TableCell onClick={() => nextButtonClick(news.id)}>
                                        <Flex direction="row" justifyContent="space-between">
                                            <Flex direction="column" gap="0rem">
                                                <h3>{news.title}</h3>
                                                <Flex style={{ color: 'gray' }}>更新日：{extractDateAndTimeChars(news.updatedAt)}</Flex>
                                            </Flex>
                                            <Flex alignItems="center" style={{ marginRight: '10px' }}>
                                                <Button variation="default" onClick={() => nextButtonClick(news.id)} size='large'>編集</Button>
                                            </Flex>
                                        </Flex>
                                    </TableCell>
                                </TableRow>
